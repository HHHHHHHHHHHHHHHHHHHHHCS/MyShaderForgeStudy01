...
////// Lighting:
                ...
                float3 finalColor = (((pow(max(0,dot(halfDirection,normalDirection)),exp2(_HighLightRange))*_HighLightColor.rgb*_HightLightIntensity*saturate((dot(_MainTexture_var.rgb,float3(0.3,0.59,0.11))*4.0+-2.0)))+(dot(normalDirection,lightDirection)*_MainTexture_var.rgb))*_LightColor0.rgb*attenuation);
